...
namespace ComapactGraphicsV2
{
    public class ExtendedColors
    {
        ...
        #endregion
        private struct ExtendedColorT
        {
            public Color color;
            public ConsoleColor backing;
            public string friendlyName;
        }

        private ExtendedColorT[] extendedColors;

        public ExtendedColors()
        {
            extendedColors = new ExtendedColorT[16];
        }

        public bool SetColor(string name, Color color, ConsoleColor inPlace)
        {
            bool isOverwrite = false;
            if (extendedColors[(int)inPlace].friendlyName == null)
                isOverwrite = true;
            extendedColors[(int)inPlace] = new ExtendedColorT() { color = color, backing = inPlace, friendlyName = name };
            SetColor(inPlace, color);
            return isOverwrite;
        }
        public ConsoleColor GetColor(string name)
        {
            for (int i = 0; i < extendedColors.Length; i++)
                if (extendedColors[i].friendlyName == name)
